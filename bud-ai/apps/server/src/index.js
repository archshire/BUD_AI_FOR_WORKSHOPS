    ...
    if (req.method === "POST" && req.url === "/api/facilitator/learning-plan") {
      return readJson(req, res, async function (body) {
        ...
        const localReply = await askLocalBud({
          ...
          question: "Generate a practical learning plan from the curriculum. Create exactly 4 numbered chapters that cover distinct learning goals or practical phases supported by the curriculum. For every chapter, use exactly three concise lines in this format: '1. Specific chapter title', 'Learner task: ...', and 'Comprehension check: ...'. Every first line must include a meaningful title after the number and period. The learner task should clearly explain what the learner will do. Use only the supplied source material. Return no preamble or closing text.",
          sourceContext: sourceContext,
          system: "You are Leader Bud, a concise workshop planning assistant for a human Leader. Create exactly 4 practical learner-centred chapters grounded only in the supplied curriculum and learning goals. Start each chapter with a number, period, and meaningful title, for example '1. Define the problem'; never return a bare chapter number. Each numbered chapter must contain one 'Learner task:' and one 'Comprehension check:' line so the Leader can review them as editable task cards. Keep every field to one concise sentence so all 4 chapters fit in the response. Do not use slides or teaching notes to create goals, and do not invent unsupported content. Return only the 4 numbered chapters.",
          max_tokens: 500,
