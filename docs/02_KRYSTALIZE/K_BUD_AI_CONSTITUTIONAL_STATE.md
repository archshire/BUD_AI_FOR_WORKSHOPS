...
| LT-043 | Bud's default local reasoning and text-translation model is Qwen3-8B. Voice input uses a separate multilingual faster-whisper `small` provider with beam size 4 for the current prototype path. Qwen runs in non-thinking mode; hardware and measured latency remain subject to benchmarking. | Human clarification, CJ-066 | 2026-07-25 | Qwen3-8B was selected for multilingual coverage, Apache 2.0 licensing, local deployment support, tool/structured-output compatibility, and a practical size/latency balance. Speech recognition is intentionally separated from language reasoning. Whisper `small` is the current quality/latency upgrade from `base`, while beam size 1 remains available for comparison. |
...
| LT-072 | Microphone input is bounded by an explicit talk turn. A participant starts with `Talk to Bud` and the facilitator with `Talk to Facil-Bud`; the active control becomes `Stop talking`. Pressing stop disables microphone publication and transcription capture. If meaningful audio is absent for 15 seconds, Bud stops the turn automatically and visibly reports the silence stop. | Human clarification | 2026-07-26 | Bud must not listen continuously by default. This preserves user agency, reduces unnecessary processing, and makes the speech-to-text boundary legible. |
| LT-073 | A completed facilitator voice transcript is forwarded to the same private Facil-Bud message path used by typed facilitator questions. It may also appear in the room transcript/caption feed, but the Facil-Bud response remains private to the facilitator. | Implementation correction | 2026-07-26 | This keeps voice and text Facil-Bud interactions behaviorally aligned while preserving the facilitator's private AI scope. |
...
| UI-005 | Measured STT latency, hardware fit, and translation quality remain subject to benchmark; the current provider default is multilingual faster-whisper `small` with beam size 4, alongside local NLLB translation. | dependency | Tier 3 | Blocks final latency claims, but no longer blocks provider-interface implementation. | benchmark and refine | Qwen3-8B is locked as the default local reasoning/text-translation model; Whisper `small` and beam size 4 are the current prototype configuration, with beam size 1 retained for comparison. | partially clarified |
