...
2. Six language options are wired into the local speech/translation path,
...
| LT-037 | If a delayed AI result no longer matches the current workshop context, Bud discards only that pending output and notifies the relevant user. Permitted transcripts, events, and relevant prior context remain available for future sensemaking. | Human clarification, CJ-065 | 2026-07-25 | Freshness takes priority over delivering an obsolete response, but context continuity takes priority over erasing prerequisite understanding. |
| LT-038 | After stale output is discarded, Bud re-anchors on the latest permitted context while retaining relevant earlier context. It may make one bounded retry when the request remains relevant and the provider is available. If prerequisite context is unclear, Bud asks for clarification or waits rather than guessing. | Human clarification, CJ-065 | 2026-07-25 | Bud must not retry indefinitely or repeatedly interrupt the workshop with recovery notifications. Exact latency budgets remain an docs/04_IMPLEMENTATION/provider clarification. |
...
| LT-043 | Bud's default local reasoning and text-translation model is Qwen3-8B. Voice input uses a separate multilingual faster-whisper `small` provider with beam size 2 for the current prototype path. Qwen runs in non-thinking mode; hardware and measured latency remain subject to benchmarking. | Human clarification, CJ-066, implementation update | 2026-07-25 | Qwen3-8B was selected for multilingual coverage, Apache 2.0 licensing, local deployment support, tool/structured-output compatibility, and a practical size/latency balance. Speech recognition is intentionally separated from language reasoning. Whisper `small` is the current quality/latency upgrade from `base`; beam size 1 remains available for comparison, and CPU threads are configurable. |
| LT-044 | Voice input uses explicit, bounded talk turns. Browser voice-activity detection closes meaningful utterances after a sustained pause, skips silent chunks, and caps continuous capture at 15 seconds. The selected native language is passed to Whisper as a hint; short-chunk language-detection disagreement does not silently delete the utterance. | Implementation update, CJ-067 | 2026-07-26 | This preserves participant agency and context continuity while reducing unnecessary STT work and latency. Raw audio remains transient and is not written to the session log. |
| LT-045 | Connected LiveKit clients subscribe to remote microphone tracks and play them locally, while the same local microphone track may be captured separately for Bud transcription. | Human-approved implementation change | 2026-07-26 | Live workshop voice communication is distinct from Bud's AI input path; raw remote or local audio is not written to the session log. |
...
| LT-068 | Bud may use the active Source Pack as shared workshop context for private learner support, Leader Bud support, translation terminology, and room-level synthesis. Learner-private Bud conversations never enter the Source Pack automatically. | Human clarification, CJ-064 | 2026-07-26 | Shared grounding enriches assistance without weakening private-context boundaries. |
...
| LT-071 | Each participant and facilitator selects a native input language during workshop setup. Bud accepts speech into the translation, event, and AI pipeline only when the recognizer detects that selected language; other detected supported languages are visibly ignored. Native input language and displayed translation language are separate choices. The prototype language set is English, Spanish, Simplified Chinese, Burmese, French, and Thai. | Human clarification | 2026-07-26 | This keeps accidental or unrelated language input from becoming workshop evidence while preserving multilingual display translation. |
| LT-072 | Microphone input is bounded by an explicit talk turn. A participant starts with `Talk to Bud` and the facilitator with `Talk to Leader Bud`; the active control becomes `Stop talking`. Pressing stop disables microphone publication and transcription capture. If meaningful audio is absent for 15 seconds, Bud stops the turn automatically and visibly reports the silence stop. | Human clarification | 2026-07-26 | Bud must not listen continuously by default. This preserves user agency, reduces unnecessary processing, and makes the speech-to-text boundary legible. |
| LT-073 | A completed facilitator voice transcript is forwarded to the same private Leader Bud message path used by typed facilitator questions. It may also appear in the room transcript/caption feed, but the Leader Bud response remains private to the facilitator. | Implementation correction | 2026-07-26 | This keeps voice and text Leader Bud interactions behaviorally aligned while preserving the facilitator's private AI scope. |
| LT-074 | Qwen is the reasoning provider, while Learner Bud and Leader Bud are distinct bounded personas defined by dedicated behavior contracts. The application selects the persona, supplies only permitted context, and validates grounding, privacy, uncertainty, freshness, and allowed actions before displaying or applying a result. If evidence is insufficient, Bud must say it does not know and ask one concise clarifying question or choose `WAIT`/`NO_ACTION`. | Human clarification / implementation lock | 2026-07-27 | Qwen supplies capability; persona contracts supply role, boundaries, tone, and authority. A generic Qwen completion is never accepted as workshop truth. |
| LT-075 | Common-room and assigned-group chat are multilingual shared context. Each message preserves the speaker's original text and language, provides recipient-specific intelligible translations as distinct content, and exposes the normalized shared event only to Buds permitted for that room/group. Private Learner Bud and Leader Bud conversations remain private unless explicitly projected. Translation failure preserves the original and shows an unavailable/uncertain state; it never fabricates a replacement. | Human clarification / KRYSTALIZE session CJ-069 | 2026-07-27 | Bud can support the workshop because it receives the permitted interaction context, while original meaning, translation, interpretation, and privacy scope remain distinct. |
| LT-076 | Bud maintains contextual continuity through a structured, room-scoped, privacy-aware memory ledger initialized from the first interaction. The ledger has default categories for Ground Context, People Index, shared chat, breakout context, Leader Bud memory, learner Bud memory, open questions, and exclusions. Each Bud interaction follows retrieve -> reason -> answer -> compact/log: retrieve only the smallest relevant permitted memory slice, answer from current runtime state and authoritative source/plan data before memory, use grounded fallback prompts when evidence is absent, and then compact the interaction into the correct scoped ledger bucket. Qwen may compact and classify events or suggest new categories, but application code validates source references, identity, room/group keys, privacy scope, staleness, and retrieval permission before writing or retrieving ledger entries. | Human clarification / KRYSTALIZE session CJ-070 | 2026-07-28 | The ledger gives Bud persistent contextual awareness without turning raw transcript dumps, full-chat replay, or model compactions into authoritative truth. Runtime state, active Source Packs, and locked learning plans remain higher authority when conflicts arise. |
...
| UI-005 | Measured STT latency, hardware fit, and translation quality remain subject to benchmark; the current provider default is multilingual faster-whisper `small` with beam size 2, alongside local NLLB translation. | dependency | Tier 3 | Blocks final latency claims, but no longer blocks provider-interface implementation. | benchmark and refine | Qwen3-8B is locked as the default local reasoning/text-translation model; Whisper `small`, beam size 2, configurable CPU threads, and bounded browser-side VAD are the current prototype configuration, with beam size 1 retained for comparison. | partially clarified |
...
| TR-052 | Bounded local speech capture | `bud-ai/apps/web/src/app/app.js`, `bud-ai/apps/stt/whisper_service.py`, `docs/01_PRODUCT/PRD_BUD_AI_DYNAMIC_MULTILINGUAL_WORKSHOPS.md` | CJ-067 | VAD, pause-based segmentation, 15-second cap, language hint, and beam-size tuning are recorded. |
| TR-054 | Multilingual common-chat context | `docs/01_PRODUCT/UI_UX_WORKSHOP_FLOW_AND_CHAT_CONTRACT.md`, `docs/03_CONTRACTS/NORMALIZED_EVENT_CONTRACT.md`, `docs/01_PRODUCT/PRD_BUD_AI_DYNAMIC_MULTILINGUAL_WORKSHOPS.md` | CJ-069 | Common chat preserves original meaning, recipient translations, Bud context visibility, and private-chat boundaries. |
| TR-053 | Remote microphone playback | `bud-ai/apps/web/src/app/app.js`, `bud-ai/apps/web/src/app/facilitator.js`, `docs/04_IMPLEMENTATION/ARCHITECTURE.md` | CJ-067 | Connected clients subscribe to and play remote LiveKit microphone tracks; raw audio remains outside the session log. |
| TR-055 | Persistent contextual memory ledger | `docs/04_IMPLEMENTATION/ARCHITECTURE.md`, `docs/03_CONTRACTS/STATE_MODEL_CONTRACT.md`, `docs/03_CONTRACTS/AI_DECISION_CONTRACT.md`, `docs/04_IMPLEMENTATION/IMPLEMENTATION_PLAN.md` | CJ-070 | Structured, privacy-scoped Markdown ledger becomes Bud's persistent contextual retrieval layer through the retrieve -> reason -> answer -> compact/log loop while runtime state remains authoritative. |
